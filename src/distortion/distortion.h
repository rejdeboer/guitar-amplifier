...
class Distortion {
   public:
    ...
    void process(const ProcessContext& context) noexcept {
        ...
        for (size_t channel = 0; channel < num_channels; ++channel) {
            for (size_t sample = 0; sample < len; ++sample) {
                ...
                //                output[sample] =
                //                high_pass_filter_.processSample(output[sample]);
            }
        }
    }
    ...
    SampleType processSample(SampleType input, int ch) noexcept {
        ...
        wet_input = kPiDivisor * std::atan(wet_input);

        auto mix = (wet_input * mix_.getNextValue()) +
                   (input * (1.0 - mix_.getNextValue()));
        return mix;
    }
   ...
   private:
    ...
    juce::dsp::IIR::Filter<float> high_pass_filter_;

    static constexpr float kPiDivisor = 2.0f / juce::MathConstants<float>::pi;
};
